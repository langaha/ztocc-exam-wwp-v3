...
import { fastReleaseQcTicket } from "@/lib/ticketService";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const body = (await req.json().catch(() => null)) as
    | { reason?: unknown; expectedVersion?: unknown; idempotencyKey?: unknown }
    | null;
  ...
  if (!reason || !Number.isFinite(expectedVersion) || expectedVersion <= 0) {
    return NextResponse.json({ error: "invalid input" }, { status: 400 });
  }

  const result = await fastReleaseQcTicket({
    ticketId: id,
    actor: user,
    expectedVersion,
    idempotencyKey,
